 ...
 */
@Component
@RabbitListener(queues = RabbitQueue.REFRESH_DATA_CHANGE_QUEUE)
public class RefreshDataChangeQueueReceiver implements Runnable{
	@Autowired
		...
		while (true) {
			if(!dimDataChangeMessageSet.isEmpty()) {
				for (String message : dimDataChangeMessageSet) {
					rabbitMQSender.send(RabbitQueue.REFRESH_AGGR_DATA_CHANGE_QUEUE, message);
