 ...
 */
@Component
@RabbitListener(queues = RabbitQueue.HIGH_PRIORITY_DATA_CHANGE_QUEUE)
public class HighPriorityChangeQueueReceiver implements Runnable{
	@Autowired
	...
	public HighPriorityChangeQueueReceiver() {
		super();
		...
		while (true) {
			if(!dimDataChangeMessageSet.isEmpty()) {
				for (String message : dimDataChangeMessageSet) {
					rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, message);
